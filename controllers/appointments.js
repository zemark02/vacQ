...
exports.addAppointment = async (req,res,next)=>{
    try{
        ...
        if(existedAppointments.length >= 3 && req.user.role !== "admin"){
            return res.status(400).json({success:true,message:`The user with ID ${res.user.id} has been vacinated more than 3 dose`})
        }
...
exports.updateAppointment =  async (req,res,next)=>{
    try{
        ...
        if(appointment.user.toSring() !== req.user.id && req.user.role != "admin"){
